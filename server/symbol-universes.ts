...
// Combined unique symbols from all indices
export const ALL_MAJOR_INDICES = Array.from(new Set([...DOW_30, ...NASDAQ_100, ...SP_500])).sort();

// Universe metadata
export const UNIVERSE_OPTIONS = [
  { id: "dow30", name: "Dow 30", description: "30 blue-chip stocks", count: DOW_30.length },
  { id: "nasdaq100", name: "Nasdaq 100", description: "100 largest Nasdaq stocks", count: NASDAQ_100.length },
  { id: "sp500", name: "S&P 500", description: "500 largest US companies", count: SP_500.length },
  { id: "all", name: "All Major Indices", description: "Combined unique stocks", count: ALL_MAJOR_INDICES.length },
] as const;

export type UniverseId = typeof UNIVERSE_OPTIONS[number]["id"];

export function getUniverseSymbols(universeId: UniverseId): string[] {
  switch (universeId) {
    case "dow30":
      return DOW_30;
    case "nasdaq100":
